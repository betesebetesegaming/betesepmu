...
export async function sendOtpHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (timedOut) {
      res.status(502).json({
        error: 'SMS gateway unreachable from cloud (connection timed out). The Africell ESME endpoint on port 5991 is not reachable from Google Cloud ΓÇö ask Africell to provide a public HTTPS API or whitelist your Cloud Functions egress IP.',
        detail: message,
      });
      return;
    }
